...
date: 2026-07-04
categories: [software, pastrami, connectome-valued functional data]
---
...
- `@points` — a numeric matrix of size $P \times 3$ (the number of points by
  the three spatial dimensions). The columns must be named `"X"`, `"Y"`, and
  `"Z"`.
- `@point_data` — a named list of **numeric-only** vectors of length $P$ (the
  number of points). Stores additional per-point attributes (e.g. fractional
  anisotropy).
- `@streamline_data` — a named list of per-streamline **scalars** (length-1
  values, any type, e.g. a mean FA or a character label).

A `bundle` stores:

- `@streamlines` — a list of `streamline` objects.
- `@streamline_data` — a named list of per-streamline vectors of length $S$
  (the number of streamlines). Attributes common to all streamlines are lifted
  here automatically at construction time and pushed back into individual
  streamlines when subsetting.
- `@bundle_data` — a named list of bundle-level **scalars** (length-1 values,
  any type).
...
- `@bundle_data` — a named list of per-bundle vectors of length $B$ (the
  ...
  automatically at construction time and pushed back into individual bundles
  when subsetting.
- `@set_data` — a named list of set-level **scalars** (length-1 values, any
  type).
