---
title: "The TDAverse"
subtitle: "Modular, interoperable, and extensible topological data analysis in R"
author:
...
affiliations:
  - id: lmjl
    name: UMR CNRS 6629, Nantes University
...
## What is topological data analysis? {visibility="hidden"}
...
See [Mémoli & Singhal (2019)](https://link.springer.com/article/10.1007/s11538-019-00614-z) for a theoretical primer that makes this precise.
...
::: {.column width="60%"}
![Created with {ggtda} and {animate}.](images/ggtda-hclust-animation.gif){fig-align="center" width="80%" .nolightbox}
:::
...
::: {.column width="60%"}
![Created with {ggtda} and {animate}.](images/ggtda-phom-animation.gif){fig-align="center" width="80%" .nolightbox}
:::
...
## Current goals {visibility="hidden"}
...
## Validation and benchmark {visibility="hidden"}
...
#| results: hold
ldeaths_swe <-
  ripserr:::numeric_to_quasi_attractor(
    ldeaths,
    data_dim = 12,
    dim_lag = 1,
    sample_lag = 1
...
::: {layout-ncol="3"}
![CRAN Webpage](images/phutil-cran-annotated.png){fig-align="center" width="66.24%" #fig-phutil-cran}

![GitHub Repository](images/phutil-github-annotated.png){fig-align="center" width="57.6%" #fig-phutil-github}

![Website](images/phutil-website-annotated.png){fig-align="center" width="49.68%" #fig-phutil-website}
:::

## The persistence class {visibility="hidden"}
...
1. The package contains data sets for demonstration and testing purposes such as `
arch_spirals
`, a list of 24 persistence diagrams computed from samples of points in the plane forming two interlocking spirals.
:::
...
- [{BH}](https://dirk.eddelbuettel.com/code/bh.html) as a `
LinkingTo
` dependency for providing access to the Boost headers for built-in Hera compilation.
...
![Sampling within two different geometries: 24 point clouds of size 1000 for each geometry.](images/phutil-distance-point-clouds.png){fig-align="center" width="63%" #fig-phutil-distance-point-clouds}
...
![An overview of the persistence set: for each sample, we show the first 5 persistence diagrams computed via `TDA::ripsDiag()` and visualized with {ggtda}.](images/phutil-distance-pds.png){fig-align="center" width="85%" #fig-phutil-distance-pds}
...
::: {.callout-note title="Tidymodels extension for persistent homology and its vectorizations"}
Current features:
...
::: {layout-ncol="2"}
![](images/tdarec-readme.png){fig-align="center" width="50%"}

![](images/tdarec-vignette.png){fig-align="center" width="50%"}
:::

## Recipe steps {visibility="hidden"}
...
## Tunable dials {visibility="hidden"}
...
- [{TDA}](https://cran.r-project.org/package=TDA): statistical significance of features in a single diagram [@fasy2014confidence;@chazal2014stochastic] and density clustering using [GUDHI](https://project.inria.fr/gudhi/software/), [Dionysus](https://www.mrzv.org/software/dionysus/) and [PHAT](https://bitbucket.org/phat-code/phat/src/master/);
- [{TDAstats}](https://cran.r-project.org/package=TDAstats): comprehensive toolset for conducting TDA [@wadhwa2018tdastats];
- [{TDAkit}](https://cran.r-project.org/package=TDAkit): ML using functional summaries of persistence homology for clustering, hypothesis testing, and visualization [@wasserman2018topological].
:::
...
- `two_sample_diagram_test()`: 
    ...
    - permutes the rows and columns of the distance matrix to compute the empirical distribution of one or more test statistics based on inter-point distances [@lovato2021multiscale];
...
- `two_sample_functional_test()`:
    ...
    - achieved using the interval-wise testing procedure of @pini2017interval.

:::

## Dependencies {visibility="hidden"}

![A schematic view of {inphr}'s dependencies.](images/inphr-deps.png){fig-align="center" width="70%"}

## Simulated point clouds {visibility="hidden"}
...
## From point clouds to diagrams {visibility="hidden"}
...
## Two-sample diagram test {visibility="hidden"}

We illustrate hypothesis testing in the space of persistence diagrams.
...
::: {.fragment .fade-in-then-semi-out}
- No loss of information when comparing persistence diagrams directly;
- However, interpretation of results is not straightforward, only a global p-value is provided.
:::

## Two-sample functional test {visibility="hidden"}

We illustrate hypothesis testing based on Betti curve summary of persistence diagrams in homological dimension 1.
...
func_test_dim1 <- two_sample_functional_test(
  ...
  x = low_noise_pers, # First sample of persistence diagrams
  y = high_noise_pers, # Second sample of persistence diagrams
  dimension = 1, # Homological dimension to consider
...
![](images/tdaverse-and-other-universes.png){fig-align="center" width="70%"}
...
::: {.callout-box .warn style="font-size:0.7em;"}
**Packages in limbo**
...
🙏 This work was funded by [an ISC grant from the R Consortium](https://r-consortium.org/all-projects/2024-group-1.html#modular-interoperable-and-extensible-topological-data-analysis-in-r) that we would like to gratefully acknowledge.
:::

![](images/isc.png){fig-align="center" width="40%"}
